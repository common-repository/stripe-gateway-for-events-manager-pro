 ...
 * @copyright 2017 Oliveconcepts.com
 */
...
class EM_Gateway_Stripe extends EM_Gateway {
		...
		function booking_form(){
			?>
            <?php _e( get_option('em_'.$this->gateway.'_form'), 'em-stripe' ); ?>
            
            <p class="em-bookings-form-gateway-cardno">
              <label>
                <?php  _e('Card Number','em-stripe'); ?>
              </label>
              <input type="text" size="15" name="stripe_card_num" value="" class="input" />
            </p>
            <p class="em-bookings-form-gateway-expiry">
              <label>
                <?php  _e('Expiration Date','em-stripe'); ?>
              </label>
              <span class="expire_date">
              <select name="stripe_exp_date_month"  style="width:150px; display:inline;">
                <?php 
				for($i = 1; $i <= 12; $i++){
					$m = $i > 9 ? $i:"0$i";
...
<?php 
		...
		function processStripe($EM_Booking){
			...
			{
				$EM_Booking->add_error(__('Please enter credit card number', 'em-stripe'));
				return false;
			}

			if(empty($_POST['stripe_exp_date_month']))
			{
				$EM_Booking->add_error(__('Please select expire month', 'em-stripe'));
				return false;
			}

			if(empty($_POST['stripe_exp_date_year']))
			{
				$EM_Booking->add_error(__('Please select expire year', 'em-stripe'));
				return false;
			}
			if(empty($_POST['stripe_card_code']))
			{
				$EM_Booking->add_error(__('Please enter CVV number', 'em-stripe'));
				return false;
			}
			if($this->debug=='yes'){
				// Send request to stripe
				EM_Pro::log(sprintf( __( 'Payment Processing Start here', 'em-stripe' )));
			}
			...
			if($this->debug=='yes'){
				EM_Pro::log(sprintf( __( 'Payment Processing start after include library', 'em-stripe' )));
			}

			Stripe::setApiKey($this->SecretKey);
			if($this->debug=='yes'){
				EM_Pro::log(sprintf( __( 'Set Secret Key', 'em-stripe' )));
			}

			try {
				$amount = $EM_Booking->get_price(false, false, true);
				if($this->debug=='yes'){
				EM_Pro::log(sprintf( __( 'Credit Card token create', 'em-stripe' )));
				}
				...
				if($this->debug=='yes'){
					EM_Pro::log(sprintf( __( 'Token genreated ID : %s', 'em-stripe'),print_r($token_id->id,true)));
				}
				...
				if($this->debug=='yes'){
					EM_Pro::log(sprintf( __( 'Return Response from Stripe: %s', 'em-stripe'),print_r($charge,true)));
				}
				...
				if($token_id->id !=''){
					if ($charge->paid == true) {
						if($this->debug=='yes'){
							EM_Pro::log(sprintf( __( 'Payment Received...', 'em-stripe' )));
						}
					...
					{
						if($this->debug=='yes'){
							EM_Pro::log(sprintf( __( 'Stripe payment failed. Payment declined.', 'em-stripe' )));
						}

						$EM_Booking->add_error(__('Stripe payment failed. Payment declined.', 'em-stripe') . ': "' . $e->getMessage() . '"');	
				...
				else
				{
					...
					if($this->debug=='yes'){
					EM_Pro::log(sprintf( __( 'Stripe payment failed. Payment declined. Please Check your Admin settings', 'em-stripe' )));
					}

					$EM_Booking->add_error(__('Stripe payment failed. Payment declined. Please Check your Admin settings', 'em-stripe') . ': "' . $e->getMessage() . '"');	
				...
				$EM_Booking->add_error(__('Connection error:', 'em-stripe') . ': "' . $e->getMessage() . '"');
				return false;
		...
		function mysettings() {
            ...
            <table class="form-table">
              <tbody>
                <tr valign="top">
                  <th scope="row"><?php _e('Success Message', 'em-stripe') ?></th>
                ...
                <tr valign="top">
                  <th scope="row"><?php _e('Success Free Message', 'em-stripe') ?></th>
                  <td><input type="text" name="_booking_feedback_free" value="<?php esc_attr_e(get_option('em_'. $this->gateway . "_booking_feedback_free" )); ?>" style='width: 40em;' />
                    <br />
                    <em>
                    <?php _e('If some cases if you allow a free ticket (e.g. pay at gate) as well as paid tickets, this message will be shown and the user will not be charged.','em-stripe'); ?>
                    </em></td>
                </tr>
              </tbody>
            </table>
            ...
            <table class="form-table">
              <tbody>
                <tr valign="top">
                  <th scope="row"><?php _e('Stripe Currency', 'em-stripe') ?></th>
                  <td><?php echo esc_html(get_option('dbem_bookings_currency','USD')); ?><br />
